...
class MerraService:
    ...
    def cleanup(self, id):
        ...
        if os.path.exists(cur_download_loc):
            shutil.rmtree(cur_download_loc)
            log.info('removed {} directory'.format(cur_download_loc))
    ...
    def convertDateRangeToList(self, startDate, endDate):
        dateList = set()
    ...
    def visualizeMerra2Image(self, id, fileNameList, varName, format):
        try:
            ...
            # AWS S3 UPLOAD
            imageFileName = f'{cur_download_loc}/image.{varName}.{int(time.time())}.png'
    ...
    def visualizeMerra2GIF(self, id, fileNameList, varName, format):
        try:
            ...
            # Save as GIF
            imageFileName = f'{cur_download_loc}/animation.{varName}.{int(time.time())}.gif'
